...
-record(targetState, {targetID,privateCert, trustPublicKey}).

init({TargetID, Certificate, TrustPublicKey}) ->
  {ok,#targetState{privateCert = Certificate, targetID = TargetID, trustPublicKey = TrustPublicKey}}.
...
handle_call({login,Username}, _From, State) ->
  ...
  Msg = #target2trust{reason = "User login",
  ...
  auth_security:verify_signature(Reply, State#targetState.trustPublicKey),
  case Reply of
